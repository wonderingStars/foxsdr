...
public:
    ...
    // pitch at widthPx is >= 152 px — the widest label the axis produces is
    // 97.52 px at the current lettering (Georgia, 14 px), an end label may
    // slide inboard by half of that to clear the frame, and 3 px of gap must
    // remain (see kMinTickSpacingPx in freq_scale.cpp for the arithmetic).
    //
