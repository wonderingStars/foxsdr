...
// WHERE A WRITTEN STACK BEGINS is not a function of this file's any more: it is
// core/crash_stack_start.hpp's crashStackStartIndex(). The rule is
// platform-free arithmetic, and it matters most on the platform where THIS file
// is not compiled at all (the root CMakeLists.txt removes
// crash_handler_posix.cpp from CASCADE_CORE under WIN32), so a definition here
// was one that could not be linked - or tested - on Windows. That header
// carries the whole argument for why the rule exists;
// crash_handler_posix.cpp's writeReport is its caller on this platform.
