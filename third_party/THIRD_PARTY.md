...
| Gradle wrapper | https://github.com/gradle/gradle | Gradle `8.7` (`gradle-wrapper.jar` + `gradlew`/`gradlew.bat`, generated by `gradle wrapper --gradle-version 8.7` from the verified distribution; committed 2026-09-16 for the Android port) | `https://services.gradle.org/distributions/gradle-8.7-bin.zip` (distribution, verified against its published `.sha256`) | distribution `544c35d6bd849ae8a5ed0bcea39ba677dc40f49df7d1835561582da2009b961d`; committed files: jar `cb0da6751c2b753a16ac168bb354870ebb1e162e9083f116729cec9c781156b8`, `gradlew` `fc977a94723af68aaffa4e5d60496fb4aeed1884b6b19e5e2f2fd7612673313d`, `gradlew.bat` `bdecf875b6868cbcbd36a1f85eedf0832f358ff28092c5797ed645f7edce77d9` | Apache-2.0 | `android/gradle/wrapper/` |
...
The only binary the Android build contributes to this repository:
`gradle-wrapper.jar`, about 43 KB, plus the `gradlew`/`gradlew.bat` launchers
that run it. It is Gradle's own bootstrapper — it downloads the pinned Gradle
distribution on first use and runs the build with it — and it is committed
rather than fetched because a build tool that must already be installed before
the build can run is a build nobody can reproduce from a clean checkout.

**Both halves of the chain are pinned.**
`gradle/wrapper/gradle-wrapper.properties` names `gradle-8.7-bin.zip` AND
carries `distributionSha256Sum`, so the wrapper refuses a distribution that
does not hash to `544c35d…`. Without that line the jar would run whatever the
URL happened to serve. The distribution itself was verified here against the
published `gradle-8.7-bin.zip.sha256` before `gradle wrapper` was run from it,
so the committed jar came out of a checked archive rather than an unverified
download.

Licensed Apache-2.0, like the rest of Gradle.

### NOT vendored: `android_native_app_glue`

Worth stating explicitly, because it is a third-party C file that the Android
build compiles and it is **not** in this directory and not in this table. It
ships inside the NDK at
`$ANDROID_NDK_HOME/sources/android/native_app_glue/`, and
`src/platform/android/CMakeLists.txt` compiles it straight from there. It is
therefore versioned by the NDK — pinned in `android/app/build.gradle` as
`ndkVersion '27.2.12479018'` — and there is no copy in this repository to drift
from upstream. Its licence (Apache-2.0) travels with the NDK, in the `NOTICE`
